...
def astar_search(
    ...
    while open:
        ...
        if pop_g > state_cost.get(pop_state, float("inf")):
            continue  # Skip this node if a cheaper path exists
        ...
        expansions += 1
        logging.info(f"Expanding node {pop_state}, cost g: {pop_g}")
        ...
        for operator, successor_state in task.get_successor_states(pop_state):
            child_node = searchspace.make_child_node(pop_node, operator, successor_state)
            child_h = heuristic(child_node)

            if child_h == float("inf"):
                continue

            if successor_state not in state_cost or child_node.g < state_cost[successor_state]:
                ...
                node_tiebreaker += 1
                state_cost[successor_state] = child_node.g
